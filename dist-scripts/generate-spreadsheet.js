 ...
 *
 * Run: npm run generate:spreadsheet
 */
...
function assignCity(prn) {
    // Special case: Dinesh gets Indore
    if (prn === '1234')
        return CITIES.find(c => c.city === 'Indore');
    const hash = crypto_1.default.createHash('sha256').update(`city:${prn}`).digest('hex');
...
async function main() {
    ...
    const outPath = path_1.default.join(process.cwd(), 'scripts', 'task3-spreadsheet.csv');
    fs_1.default.writeFileSync(outPath, csv, 'utf8');
